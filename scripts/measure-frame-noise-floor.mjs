...
async function runOnce(frames, playerExe) {
  ...
  try {
    await waitForDebuggerReady(PORT, 20000);
    ...
    try {
      ...
      return JSON.parse(dataResult.result.value);
    } finally {
      await client.close();
    }
  } finally {
...
function renderMarkdown(opts, perRunStats, spread, resolution) {
...
is ${resolution.width}x${resolution.height}. ${opts.frames} frames per run, first
...
async function main() {
  ...
  try {
    for (let i = 0; i < opts.runs; i++) {
      ...
      const { frameTimes, resolution } = await runOnce(opts.frames, playerExe);
  ...
  const markdown = renderMarkdown(opts, perRunStats, spread, lastResolution);
